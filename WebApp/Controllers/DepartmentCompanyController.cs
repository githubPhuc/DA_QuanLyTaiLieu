    ...
    public class DepartmentCompanyController : BaseController
    {
        ...
        public async Task<ActionResult> GetList(string NameDepartmentSearch)
        {
            ...
            var data = await(from a in _Departments
                        ...
                        where a.isDelete == false && d.ParentId ==ParentCompany                             
        ...
        {
            var dataConfig = db_.Configs.Where(a => a.ParentId == ParentCompany && a.isDelete == false).OrderByDescending(a => a.DatetimeCreate).Select(a => new ConfigModel { Id = a.Id, Name = a.NameConfig, Description = a.DescriptionConfig }).ToList();
            ViewBag.datacompany = dataConfig;
            return PartialView();
        }
        public JsonResult LoadCompany()
        {
            var dataConfig = db_.Configs.Where(a => a.ParentId == ParentCompany && a.isDelete == false).OrderByDescending(a => a.DatetimeCreate)
        ...
        public ActionResult Edit(int id)
        {
            ...
            ViewBag.data = data;
            var dataConfig = db_.Configs.Where(a => a.ParentId == ParentCompany && a.isDelete == false).Select(a=>new ConfigModel { Id = a.Id,Name = a.NameConfig,Description = a.DescriptionConfig}).ToList();
            ViewBag.datacompany = dataConfig;
            return PartialView();
        }
        [ValidateInput(false)]
