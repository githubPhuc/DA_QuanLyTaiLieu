...
{
    [Authorize]
    public class OutgoingDocumentController : BaseController
    {
        // GET: CartManagerment
        ...
        public async Task<ActionResult> GetList(string NameTitleSearch, string DatetimeCreateStartSearch, string DatetimeCreateEndSearch)
        {
            ...
            var DataDeliverys = await (from a in _Deliverys
                                       ...
                                       where a.isDelete == false && d.Id == appglobal_.ID_TEXT_OUTGOING && 
            ...
            var result = (from a in DataDeliverys
                          ...
                          .OrderByDescending(a => a.DatetimeCreate).ThenBy(a => a.DepartmentName).ToList();
        ...
        public async Task<ActionResult> Edit(int Id)
        {
            ...
            if (data != null)
            {
                ...
                var result = (from a in db_.Departments
                              ...
                              where !LstDeliverys.Contains(a.Id) && a.isDelete == false && a.ParentConFig != null
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (dataCopy != null)
                    {
                        ...
                        model.ParentId = appglobal_.ID_TEXT_OUTGOING;
        ...
        public JsonResult _DeleteFun(int id)
        {
            ...
            try
            {
                ...
                if (data != null)
                {
                    ...
                    appGlobal.AddRecycleBin(data.Id, "Xóa data chương trình quản lý văn bản nội bộ", "", "Delivery", User.UserId);
        ...
        public async Task<ActionResult> Insert()
        {
            ...
            var RecipientAddress = await (from a in _Departments
                                          join b in _Configs on a.ParentConFig equals b.Id
                                          where b.ParentId == appglobal_.ID_COMPANY_PARTNER && a.isDelete == false
                                          select new ConfigModel
                                          {
                                              Id = a.Id,
            ...
            var us = await db_.Users.Where(a => a.isDelete == false).ToListAsync();
            ViewBag.Userss = us;

            return View();
        }
        public async Task<JsonResult> LoadUserByDepartment()
        {
                ...
                if (model.RecipientAddressId > 0)
                {
                    if (model.files.Count() > 0)
                    {
                        if (!string.IsNullOrEmpty(model.TitleDocument))
                        {
                                ...
                                if (!string.IsNullOrEmpty(model.AbstractDocument))
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(model.DatePublish))
                                    {
                                        if (model.FolderId > 0)
                                        {
                                            ...
                                            {
                                                ParentId = appglobal_.ID_TEXT_OUTGOING,
                            ...
                            {
                                return Json(new { status = -1, title = "", text = "Không tìm thấy dữ liệu phòng ban soạn thả", obj = "" }, JsonRequestBehavior.AllowGet);
                            }
                        }
                        else
                        {
